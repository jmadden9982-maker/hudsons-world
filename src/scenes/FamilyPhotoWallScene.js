...
export default class FamilyPhotoWallScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (hasArt) {
      const bg = this.add.image(W/2, H/2, 'bg_photowall');
    ...
    this.add.text(W/2, 26, '📸 Family Photo Wall', {
      fontFamily: FONT,
      fontSize: '26px',
      color: '#FFE9C9',
      fontStyle: 'bold'
    ...
    this.add.text(W/2, 56, photosUnlocked(TOTAL) + ' / ' + TOTAL + ' photos unlocked — tap one to see it big!', {
      fontFamily: FONT,
      fontSize: '14px',
      color: '#FFE9C9'
    }).setOrigin(0.5);
    ...
    for (let i = 0; i < TOTAL; i++) {
      ...
      if (unlocked && this.textures.exists('photo' + i)) {
        // === SAFE OPTIONAL: Use painted polaroid frame if present ===
        if (this.textures.exists('ui_polaroid_frame')) {
          ...
          // Fallback drawn frame (original behaviour)
